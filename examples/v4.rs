...
fn parse_cities(path: PathBuf) -> Result<HashMap<String, Price>> {
...
fn main() -> Result<()> {
    let path = PathBuf::from("hotels.csv");
...
mod tests {
    ...
    fn test_parse_cities() {
        ...
        let path = dir.path().join("hotels.csv");
        let mut file = File::create(&path).unwrap();
        file.write_all(
            b"Hotel;City;Date;RoomType;Price\n\
